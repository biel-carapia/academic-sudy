...
# define uma string create_table que contém um comando SQL #para criar uma tabela chamada "Produto".
# Esta tabela terá quatro colunas: id (chave primária), nome #(texto), preco (número real) e estoque (numero inteiro).
# O IF NOT EXISTS garante que a tabela só terá criada se ainda #não existir
create_table = """
...
# Usa o método execute do objeto cursor para executar o #comando SQL definido anteriormente e criar a tabela no banco 3de dados.
...
# Finalmente, você usa o método close no objeto de conexão para encerrar a conexão com banco de dados.
